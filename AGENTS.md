...
- **Environment**: the Docker image `koalalive/cmdstanr4cogneuro:1.0.1`
...
For an interactive IDE:

```bash
docker run -d --name ewmv-rstudio -p 8787:8787 \
  -e RS_PASSWORD=rstudio -v "$(pwd)":/root/stan \
  koalalive/cmdstanr4cogneuro:1.0.1 \
  bash /root/stan/_scripts/start_rstudio.sh
# then open http://localhost:8787  (user: rstudio-server / password: RS_PASSWORD)
```
...
4. **RStudio login**: username `rstudio-server`; password = `RS_PASSWORD` env
   (default `rstudio`). `_scripts/start_rstudio.sh` patches the image (libR
   library paths, `/home/rstudio-server`, PAM config, password) before launch.
...
| `_scripts/start_rstudio.sh` | RStudio Server bootstrap (see above) |
